...
pub enum Error {
    ...
    InvalidAddress = 12,
    /// The escrow has been administratively paused; normal operations are
    /// blocked until the admin resumes it via `admin_resume_escrow`.
    EscrowPaused = 13,
    /// The yield rate basis-points value supplied to `admin_set_yield_rate`
    /// exceeds the allowed maximum (10 000 bps = 100 %).
    YieldRateInvalid = 14,
}
